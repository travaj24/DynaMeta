...
m_c(T_e) -> wp(T_e) and gamma(T_e) RATIO tables that the 2D-TE NumPy FDTD reference kernel
...
The hot electrons climb the nonparabolic Kane band so the f-sum (conductivity) mass m_c(T_e) RISES
-> wp^2 ~ n/m_c DROPS (Re(eps) moves toward eps_inf, through ENZ) and scatter more so gamma(T_e)
RISES. wp^2 = n e^2/(eps0 m) is an f-SUM-RULE quantity, so the mass here is carrier_heating's f-sum
conductivity mass, NOT a mass evaluated at the mean energy (audit C-1: the pre-fix mean-energy mass
ran 17.2% low at the ITO preset, inflating every wp table by 20.8%). wp and gamma are carried as
material-only RATIO tables anchored at the cold electron temperature T_e0, so a cell's cold
(wp0, gamma0) reproduce EXACTLY at T_e = T_e0 (off-switch consistency):
    wp(T_e)    = wp0    * sqrt(m_c(T_e0) / m_c(T_e)),
    gamma(T_e) = gamma0 * (T_e / T_e0)^p.
Off-switches:
    alpha_per_eV == 0 -> m_c == m0 constant -> wp ratio == 1 (no plasma shift, exact);
...
class HotCarrierParams:
    ...
    volumetric power density W/m^3). `n_m3`, `m0_kg`, `alpha_per_eV` set the Kane f-sum mass that
    fixes the m_c(T_e) -> wp(T_e) shift; the cell's COLD (wp0, gamma0) come from the FDTDLayer's own
    ...
    Fields (SI):
      ...
      n_m3          carrier density (fixed; sets E_F / mu(T_e) for the Kane f-sum mass)
...
def build_hot_carrier_tables(hc: HotCarrierParams) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    np.interp:
      ...
      wp_ratio (n_table,)  = sqrt(m_c(T_e0)/m_c(T_e))    (multiplies the cell's cold wp0),
    ...
    # m_c(T_e) is the f-SUM (conductivity) mass wp^2 = n e^2/(eps0 m) requires (audit C-1). The
    # exact Fermi-Dirac integral behind it is valid degenerate AND nondegenerate, so the high-T tail
    # of the TABLE needs no validity suppression -- the pre-C-1 Sommerfeld expansion did (its
    # RuntimeWarning, and the catch_warnings block that muted it here, are both gone).
    ...
    wp_ratio = np.sqrt(m_ref / m_star)                       # wp ~ 1/sqrt(m_c) -> DROPS as T_e rises
