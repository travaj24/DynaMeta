...
def _density(cf):
    return cf.regions["semi"].grid_fields[ELECTRON_DENSITY][0, 0, :]


def peak_and_sheet(cf, v0):
    """peak n/n_bg, and the net excess sheet density RELATIVE TO THE FLAT-BAND profile
    v0 (psi_s=0) -- not 2*n_bg. Baselining against n_bg counted the hard-wall dead-layer
    deficit as 'negative accumulation' and broke the series-cap relation (audit SP-2)."""
    v = _density(cf)
    z = cf.regions["semi"].grid_axes_m["z"]
    n_exc = float(np.sum(0.5 * ((v[:-1] + v[1:]) - (v0[:-1] + v0[1:])) * np.diff(z)))
...
def main():
    ...
    # flat-band (Vg=0) baseline density -- the reference for the excess sheet charge
    v0 = _density(phys.solve(BiasPoint({"gate": 0.0, "body": 0.0}, "flat")))
    ...
    pk_n, ne_n = peak_and_sheet(cf_n, v0)
    pk_p, ne_p = peak_and_sheet(cf_p, v0)
    C_ox = EPS_OX * EPS0 / T_OX
    consistency = psi_p + Q * ne_p / C_ox                       # should equal Vg (flat-band baseline)
    ...
    divides = psi_p < 0.9 * VG                                   # meaningful oxide drop
    less_accum = 0.0 < ne_p < 0.8 * ne_n                          # physical accumulation positive but smaller
    consistent = abs(consistency - VG) < 0.05 * VG               # series-cap relation satisfied (Gauss)
