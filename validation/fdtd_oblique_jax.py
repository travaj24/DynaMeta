...
def main():
    ...
    from dynameta.optics.fdtd_nd import _run_2d_tm_oblique_jax, _run_3d_oblique_jax
    nx2, nz2 = 4, 70
    ...
    cp2 = _cpml_z(nz2, dz2, dt2, 10)
    kx2 = (2 * np.pi / 1.5e-6) * math.sin(math.radians(30.0))

    def lossTM(s):
        eps = 1.0 + s * jnp.asarray(band2) * 3.0
        out = _run_2d_tm_oblique_jax(eps, jnp.zeros((nx2, nz2)), jnp.zeros((nx2, nz2)), dx2, dz2, dt2,
    ...
    cp3 = _cpml_z(nz3, dz3, dt3, 8)
    kx3 = (2 * np.pi / 1.5e-6) * math.sin(math.radians(20.0))

    def loss3D(s):
        eps = 1.0 + s * jnp.asarray(band3) * 3.0
        out = _run_3d_oblique_jax(eps, jnp.zeros((nx3, ny3, nz3)), jnp.zeros((nx3, ny3, nz3)), dx3, dy3,
