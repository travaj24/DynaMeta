"""Validate CONICAL incidence (azimuth phi != 0), p-pol (TM). An isotropic LAYERED stack is
azimuthally symmetric, so at a fixed polar angle theta the R/T must be INDEPENDENT of phi and equal
to tmm(theta, 'p'). This exercises the p-pol conical machinery: the in-plane p-pol field split over
(cos phi, sin phi) with the z-component carrying the full |k_par|, the 2D Bloch phase, and the
2D-demodulated p-pol extraction (project onto (cos phi, sin phi), demodulate by (kx, ky)). At phi=0
this reduces byte-identically to the validated p-pol oblique path. Test air/slab(n=2,250nm)/air at
theta=30deg over phi=0/30/60/90deg vs tmm('p'). Run: python -m validation.conical_ppol_vs_tmm
"""
import sys, os, math
import numpy as np
...
def main():
    ...
    d = Design(name="conical_p", unit_cell=cell, stack=stack, electrodes=[], materials=reg, mesh_3d=m3)
    ...
    ref = tmm.coh_tmm('p', [1.0, complex(N_SLAB), 1.0], [np.inf, D, np.inf], math.radians(THETA), LAM)
    Rt, Tt = ref['R'], ref['T']
    print("[t] CONICAL p-pol, theta={:.0f}deg, vs tmm R={:.4f} T={:.4f} (phi-invariant for a layered stack)".format(
        THETA, Rt, Tt), flush=True)
    ok = True
    Rs = []
    for phi in PHIS:
        opt = OpticalSpec(polarization="p", incidence_angle_deg=THETA, azimuth_deg=phi,
    ...
    # phi-spread is a SAME-MESH azimuthal difference for an isotropic stack -> it should be at
    # quadrature-noise level, far below the per-phi tmm tolerance. Gate it tightly so a transverse
    # p-pol-split / extraction defect cannot hide under the loose 0.03.
    SPREAD_TOL = 5e-3
    ...
    print("[t] *** CONICAL INCIDENCE (p-pol, phi-invariance + tmm): {} ***".format("PASS" if ok else "FAIL"),
           flush=True)
    return ok
